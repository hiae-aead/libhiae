...
update_state_offset(DATA128b *state, DATA128b *tmp, DATA128b M, int offset)
{
    ...
    DATA128b mask_t = SIMD_XOR(tmp[offset], M); // Compute mask 't'
...
{
    DATA128b mask_t = XAESL(state[(P_0 + offset) % STATE], state[(P_1 + offset) % STATE]);
    mask_t          = SIMD_XOR(mask_t, M); // This is the mask 't'
    state[(0 + offset) % STATE] = SIMD_XOR(mask_t, AESL(state[(P_4 + offset) % STATE]));
...
dec_offset(DATA128b *state, DATA128b *tmp, DATA128b C, int offset)
{
    ...
    state[(I_2 + offset) % STATE] =
        SIMD_XOR(state[(I_2 + offset) % STATE], mask_t); // HiAEt: use mask instead of M
    DATA128b M = SIMD_XOR(mask_t, tmp[offset]); // Recover original plaintext
    return M;
}
...
static void
HiAEt_init_arm_sha3(HiAEt_state_t *state_opaque, const uint8_t *key, const uint8_t *nonce)
{
...
static void
HiAEt_absorb_arm_sha3(HiAEt_state_t *state_opaque, const uint8_t *ad, size_t len)
{
    DATA128b state[STATE];
...
static void
HiAEt_finalize_arm_sha3(HiAEt_state_t *state_opaque,
                        uint64_t       ad_len,
                        uint64_t       msg_len,
                        uint8_t       *tag)
{
...
static void
HiAEt_enc_arm_sha3(HiAEt_state_t *state_opaque, uint8_t *ci, const uint8_t *mi, size_t size)
{
    DATA128b state[STATE];
    memcpy(state, state_opaque->opaque, sizeof(state));
...
static void
HiAEt_dec_arm_sha3(HiAEt_state_t *state_opaque, uint8_t *mi, const uint8_t *ci, size_t size)
{
    DATA128b state[STATE];
    memcpy(state, state_opaque->opaque, sizeof(state));
    size_t rest   = size % UNROLL_BLOCK_SIZE;
...
static void
HiAEt_enc_partial_noupdate_arm_sha3(HiAEt_state_t *state_opaque,
                                    uint8_t       *ci,
                                    const uint8_t *mi,
                                    size_t         size)
{
...
static void
HiAEt_dec_partial_noupdate_arm_sha3(HiAEt_state_t *state_opaque,
                                    uint8_t       *mi,
                                    const uint8_t *ci,
                                    size_t         size)
{
...
static int
HiAEt_encrypt_arm_sha3(const uint8_t *key,
                       const uint8_t *nonce,
                       const uint8_t *msg,
                       uint8_t       *ct,
                       size_t         msg_len,
...
{
    HiAEt_state_t state;
    HiAEt_init_arm_sha3(&state, key, nonce);
    HiAEt_absorb_arm_sha3(&state, ad, ad_len);
    HiAEt_enc_arm_sha3(&state, ct, msg, msg_len);
    HiAEt_finalize_arm_sha3(&state, ad_len, msg_len, tag);

    return 0;
}

static int
HiAEt_decrypt_arm_sha3(const uint8_t *key,
                       const uint8_t *nonce,
                       uint8_t       *msg,
                       const uint8_t *ct,
                       size_t         ct_len,
                       ...
                       const uint8_t *tag)
{
    ...
    HiAEt_init_arm_sha3(&state, key, nonce);
    HiAEt_absorb_arm_sha3(&state, ad, ad_len);
    HiAEt_dec_arm_sha3(&state, msg, ct, ct_len);
    HiAEt_finalize_arm_sha3(&state, ad_len, ct_len, computed_tag);
    ...
    const uint8_t *key, const uint8_t *nonce, const uint8_t *data, size_t data_len, uint8_t *tag)
{
    HiAEt_state_t state;
    HiAEt_init_arm_sha3(&state, key, nonce);
    HiAEt_absorb_arm_sha3(&state, data, data_len);
    HiAEt_finalize_arm_sha3(&state, data_len, 0, tag);

    return 0;
}

const HiAEt_impl_t hiaet_arm_sha3_impl = { .name     = "ARM SHA3",
                                           .init     = HiAEt_init_arm_sha3,
                                           .absorb   = HiAEt_absorb_arm_sha3,
                                           .finalize = HiAEt_finalize_arm_sha3,
                                           .enc      = HiAEt_enc_arm_sha3,
                                           .dec      = HiAEt_dec_arm_sha3,
                                           .enc_partial_noupdate =
                                               HiAEt_enc_partial_noupdate_arm_sha3,
                                           .dec_partial_noupdate =
                                               HiAEt_dec_partial_noupdate_arm_sha3,
                                           .encrypt = HiAEt_encrypt_arm_sha3,
                                           .decrypt = HiAEt_decrypt_arm_sha3,
                                           .mac     = HiAEt_mac_arm_sha3 };
...
const HiAEt_impl_t hiaet_arm_sha3_impl = { .name                 = NULL,
                                           .init                 = NULL,
                                           .absorb               = NULL,
                                           .finalize             = NULL,
                                           .enc                  = NULL,
