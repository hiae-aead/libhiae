...
static void
HiAEt_init_aesni(HiAEt_state_t *state_opaque, const uint8_t *key, const uint8_t *nonce)
{
...
static void
HiAEt_absorb_aesni(HiAEt_state_t *state_opaque, const uint8_t *ad, size_t len)
{
    DATA128b state[STATE];
...
static void
HiAEt_finalize_aesni(HiAEt_state_t *state_opaque, uint64_t ad_len, uint64_t msg_len, uint8_t *tag)
{
    DATA128b state[STATE];
    memcpy(state, state_opaque->opaque, sizeof(state));
    uint64_t lens[2];
...
static void
HiAEt_enc_aesni(HiAEt_state_t *state_opaque, uint8_t *ci, const uint8_t *mi, size_t size)
{
    DATA128b state[STATE];
    memcpy(state, state_opaque->opaque, sizeof(state));
...
static void
HiAEt_dec_aesni(HiAEt_state_t *state_opaque, uint8_t *mi, const uint8_t *ci, size_t size)
{
    DATA128b state[STATE];
    memcpy(state, state_opaque->opaque, sizeof(state));
    size_t rest   = size % UNROLL_BLOCK_SIZE;
...
static void
HiAEt_enc_partial_noupdate_aesni(HiAEt_state_t *state_opaque,
                                 uint8_t       *ci,
                                 const uint8_t *mi,
                                 size_t         size)
{
...
static void
HiAEt_dec_partial_noupdate_aesni(HiAEt_state_t *state_opaque,
                                 uint8_t       *mi,
                                 const uint8_t *ci,
                                 size_t         size)
{
...
static int
HiAEt_encrypt_aesni(const uint8_t *key,
                    const uint8_t *nonce,
                    const uint8_t *msg,
                    uint8_t       *ct,
                    size_t         msg_len,
...
{
    HiAEt_state_t state;
    HiAEt_init_aesni(&state, key, nonce);
    HiAEt_absorb_aesni(&state, ad, ad_len);
    HiAEt_enc_aesni(&state, ct, msg, msg_len);
    HiAEt_finalize_aesni(&state, ad_len, msg_len, tag);

    return 0;
}

static int
HiAEt_decrypt_aesni(const uint8_t *key,
                    const uint8_t *nonce,
                    uint8_t       *msg,
                    const uint8_t *ct,
                    size_t         ct_len,
                    ...
                    const uint8_t *tag)
{
    ...
    HiAEt_init_aesni(&state, key, nonce);
    HiAEt_absorb_aesni(&state, ad, ad_len);
    HiAEt_dec_aesni(&state, msg, ct, ct_len);
    HiAEt_finalize_aesni(&state, ad_len, ct_len, computed_tag);
    ...
    const uint8_t *key, const uint8_t *nonce, const uint8_t *data, size_t data_len, uint8_t *tag)
{
    HiAEt_state_t state;
    HiAEt_init_aesni(&state, key, nonce);
    HiAEt_absorb_aesni(&state, data, data_len);
    HiAEt_finalize_aesni(&state, data_len, 0, tag);

    return 0;
}

const HiAEt_impl_t hiaet_aesni_impl = { .name                 = "AES-NI",
                                        .init                 = HiAEt_init_aesni,
                                        .absorb               = HiAEt_absorb_aesni,
                                        .finalize             = HiAEt_finalize_aesni,
                                        .enc                  = HiAEt_enc_aesni,
                                        .dec                  = HiAEt_dec_aesni,
                                        .enc_partial_noupdate = HiAEt_enc_partial_noupdate_aesni,
                                        .dec_partial_noupdate = HiAEt_dec_partial_noupdate_aesni,
                                        .encrypt              = HiAEt_encrypt_aesni,
                                        .decrypt              = HiAEt_decrypt_aesni,
                                        .mac                  = HiAEt_mac_aesni };
...
#else
// x86 not available, provide stub implementation
const HiAEt_impl_t hiaet_aesni_impl = { .name                 = NULL,
                                        .init                 = NULL,
                                        .absorb               = NULL,
                                        .finalize             = NULL,
                                        .enc                  = NULL,
