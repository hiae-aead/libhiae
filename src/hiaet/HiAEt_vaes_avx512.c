...
update_state_offset(DATA128b *state, DATA128b *tmp, DATA128b M, int offset)
{
    ...
    DATA128b mask_t = AESENC(tmp[offset], M); // This is the mask 't'
    state[(0 + offset) % STATE] = AESENC(state[(P_4 + offset) % STATE], mask_t);
    state[(I_1 + offset) % STATE] =
        SIMD_XOR(state[(I_1 + offset) % STATE], mask_t); // HiAEt: use mask instead of M
    state[(I_2 + offset) % STATE] =
        SIMD_XOR(state[(I_2 + offset) % STATE], mask_t); // HiAEt: use mask instead of M
}
...
{
    // HiAEt modification: compute mask 't' and use it for state updates
    DATA128b mask_t = SIMD_XOR(state[(P_0 + offset) % STATE], state[(P_1 + offset) % STATE]);
    mask_t          = AESENC(mask_t, M); // This is the mask 't'
    state[(0 + offset) % STATE] = AESENC(state[(P_4 + offset) % STATE], mask_t);
    DATA128b C = SIMD_XOR(mask_t, state[(P_7 + offset) % STATE]); // Ciphertext output
    state[(I_1 + offset) % STATE] =
        SIMD_XOR(state[(I_1 + offset) % STATE], mask_t); // HiAEt: use mask instead of M
    state[(I_2 + offset) % STATE] =
        SIMD_XOR(state[(I_2 + offset) % STATE], mask_t); // HiAEt: use mask instead of M
...
dec_offset(DATA128b *state, DATA128b *tmp, DATA128b C, int offset)
{
    ...
    state[(I_2 + offset) % STATE] =
        SIMD_XOR(state[(I_2 + offset) % STATE], mask_t); // HiAEt: use mask instead of M
    DATA128b plaintext = AESENC(tmp[offset], mask_t); // Recover original plaintext
    return plaintext;
}
...
/* High-level functions with VAES and streaming optimizations */
...
typedef __m512i VAES_state_t;

/* Store 16 128-bit values into a 512-bit structure for VAES operations */
static inline VAES_state_t
pack_state_vaes(__m128i *state)
{
    __m256i lo = _mm256_loadu_si256((__m256i *) state);
    __m256i hi = _mm256_loadu_si256((__m256i *) (state + 2));
    return _mm512_inserti64x4(_mm512_castsi256_si512(lo), hi, 1);
}

/* Extract 16 128-bit values from a 512-bit structure */
static inline void
unpack_state_vaes(__m128i *state, VAES_state_t packed)
{
    __m256i lo = _mm512_extracti64x4_epi64(packed, 0);
    __m256i hi = _mm512_extracti64x4_epi64(packed, 1);
    _mm256_storeu_si256((__m256i *) state, lo);
    _mm256_storeu_si256((__m256i *) (state + 2), hi);
}
...
static void
HiAEt_init_vaes_avx512(HiAEt_state_t *state_opaque, const uint8_t *key, const uint8_t *nonce)
{
...
static void
HiAEt_absorb_vaes_avx512(HiAEt_state_t *state_opaque, const uint8_t *ad, size_t len)
{
    DATA128b state[STATE];
...
static void
HiAEt_finalize_vaes_avx512(HiAEt_state_t *state_opaque,
                           uint64_t       ad_len,
                           uint64_t       msg_len,
                           uint8_t       *tag)
{
...
static void
HiAEt_enc_vaes_avx512(HiAEt_state_t *state_opaque, uint8_t *ci, const uint8_t *mi, size_t size)
{
    DATA128b state[STATE];
    memcpy(state, state_opaque->opaque, sizeof(state));
...
static void
HiAEt_dec_vaes_avx512(HiAEt_state_t *state_opaque, uint8_t *mi, const uint8_t *ci, size_t size)
{
    DATA128b state[STATE];
    memcpy(state, state_opaque->opaque, sizeof(state));
    size_t rest   = size % UNROLL_BLOCK_SIZE;
...
static void
HiAEt_enc_partial_noupdate_vaes_avx512(HiAEt_state_t *state_opaque,
                                       uint8_t       *ci,
                                       const uint8_t *mi,
                                       size_t         size)
{
...
static void
HiAEt_dec_partial_noupdate_vaes_avx512(HiAEt_state_t *state_opaque,
                                       uint8_t       *mi,
                                       const uint8_t *ci,
                                       size_t         size)
{
...
static int
HiAEt_encrypt_vaes_avx512(const uint8_t *key,
                          const uint8_t *nonce,
                          const uint8_t *msg,
                          uint8_t       *ct,
                          size_t         msg_len,
...
{
    HiAEt_state_t state;
    HiAEt_init_vaes_avx512(&state, key, nonce);
    HiAEt_absorb_vaes_avx512(&state, ad, ad_len);
    HiAEt_enc_vaes_avx512(&state, ct, msg, msg_len);
    HiAEt_finalize_vaes_avx512(&state, ad_len, msg_len, tag);

    return 0;
}

static int
HiAEt_decrypt_vaes_avx512(const uint8_t *key,
                          const uint8_t *nonce,
                          uint8_t       *msg,
                          const uint8_t *ct,
                          size_t         ct_len,
...
{
    HiAEt_state_t state;
    uint8_t       computed_tag[HIAET_MACBYTES];
    HiAEt_init_vaes_avx512(&state, key, nonce);
    HiAEt_absorb_vaes_avx512(&state, ad, ad_len);
    HiAEt_dec_vaes_avx512(&state, msg, ct, ct_len);
    HiAEt_finalize_vaes_avx512(&state, ad_len, ct_len, computed_tag);

    return hiaet_constant_time_compare(computed_tag, tag, HIAET_MACBYTES);
}

static int
HiAEt_mac_vaes_avx512(
    const uint8_t *key, const uint8_t *nonce, const uint8_t *data, size_t data_len, uint8_t *tag)
{
    HiAEt_state_t state;
    HiAEt_init_vaes_avx512(&state, key, nonce);
    HiAEt_absorb_vaes_avx512(&state, data, data_len);
    HiAEt_finalize_vaes_avx512(&state, data_len, 0, tag);

    return 0;
}

const HiAEt_impl_t hiaet_vaes_avx512_impl = { .name     = "VAES+AVX512",
                                              .init     = HiAEt_init_vaes_avx512,
                                              .absorb   = HiAEt_absorb_vaes_avx512,
                                              .finalize = HiAEt_finalize_vaes_avx512,
                                              .enc      = HiAEt_enc_vaes_avx512,
                                              .dec      = HiAEt_dec_vaes_avx512,
                                              .enc_partial_noupdate =
                                                  HiAEt_enc_partial_noupdate_vaes_avx512,
                                              .dec_partial_noupdate =
                                                  HiAEt_dec_partial_noupdate_vaes_avx512,
                                              .encrypt = HiAEt_encrypt_vaes_avx512,
                                              .decrypt = HiAEt_decrypt_vaes_avx512,
                                              .mac     = HiAEt_mac_vaes_avx512 };
...
const HiAEt_impl_t hiaet_vaes_avx512_impl = { .name                 = NULL,
                                              .init                 = NULL,
                                              .absorb               = NULL,
                                              .finalize             = NULL,
                                              .enc                  = NULL,
