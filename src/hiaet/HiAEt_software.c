...
// Always compile software implementation for HiAEt reference
...
update_state_offset(DATA128b *state, DATA128b *tmp, DATA128b M, int offset)
{
    ...
    DATA128b mask_t = SIMD_XOR(tmp[offset], M); // This is the computed mask 't'
    state[(0 + offset) % STATE] = SIMD_XOR(mask_t, AESL(state[(P_4 + offset) % STATE]));
    state[(I_1 + offset) % STATE] =
        SIMD_XOR(state[(I_1 + offset) % STATE], mask_t); // Use mask_t instead of M
    state[(I_2 + offset) % STATE] =
        SIMD_XOR(state[(I_2 + offset) % STATE], mask_t); // Use mask_t instead of M
}
...
enc_offset(DATA128b *state, DATA128b M, int offset)
{
    ...
    DATA128b mask_t = SIMD_XOR(C, M); // This is our mask 't'
    state[(0 + offset) % STATE] = SIMD_XOR(mask_t, AESL(state[(P_4 + offset) % STATE]));
    C                           = SIMD_XOR(mask_t, state[(P_7 + offset) % STATE]); // Ciphertext
    state[(I_1 + offset) % STATE] =
        SIMD_XOR(state[(I_1 + offset) % STATE], mask_t); // Use mask_t instead of M
    state[(I_2 + offset) % STATE] =
        SIMD_XOR(state[(I_2 + offset) % STATE], mask_t); // Use mask_t instead of M
...
{
    tmp[offset]   = XAESL(state[(P_0 + offset) % STATE], state[(P_1 + offset) % STATE]);
    DATA128b temp = SIMD_XOR(state[(P_7 + offset) % STATE], C);
    state[(0 + offset) % STATE]   = SIMD_XOR(temp, AESL(state[(P_4 + offset) % STATE]));
    DATA128b mask_t               = temp; // mask_t equals the recovered mask from encryption
...
static void
HiAEt_init_software(HiAEt_state_t *state_opaque, const uint8_t *key, const uint8_t *nonce)
{
...
static void
HiAEt_absorb_software(HiAEt_state_t *state_opaque, const uint8_t *ad, size_t len)
{
    DATA128b state[STATE];
...
static void
HiAEt_finalize_software(HiAEt_state_t *state_opaque,
                        uint64_t       ad_len,
                        uint64_t       msg_len,
                        uint8_t       *tag)
{
...
static void
HiAEt_enc_software(HiAEt_state_t *state_opaque, uint8_t *ci, const uint8_t *mi, size_t size)
{
    DATA128b state[STATE];
    memcpy(state, state_opaque->opaque, sizeof(state));
...
static void
HiAEt_dec_software(HiAEt_state_t *state_opaque, uint8_t *mi, const uint8_t *ci, size_t size)
{
    DATA128b state[STATE];
    memcpy(state, state_opaque->opaque, sizeof(state));
    size_t rest   = size % UNROLL_BLOCK_SIZE;
...
static void
HiAEt_enc_partial_noupdate_software(HiAEt_state_t *state_opaque,
                                    uint8_t       *ci,
                                    const uint8_t *mi,
                                    size_t         size)
{
...
static void
HiAEt_dec_partial_noupdate_software(HiAEt_state_t *state_opaque,
                                    uint8_t       *mi,
                                    const uint8_t *ci,
                                    size_t         size)
{
...
static int
HiAEt_encrypt_software(const uint8_t *key,
                       const uint8_t *nonce,
                       const uint8_t *msg,
                       uint8_t       *ct,
                       size_t         msg_len,
...
{
    HiAEt_state_t state;
    HiAEt_init_software(&state, key, nonce);
    HiAEt_absorb_software(&state, ad, ad_len);
    HiAEt_enc_software(&state, ct, msg, msg_len);
    HiAEt_finalize_software(&state, ad_len, msg_len, tag);

    return 0;
}

static int
HiAEt_decrypt_software(const uint8_t *key,
                       const uint8_t *nonce,
                       uint8_t       *msg,
                       const uint8_t *ct,
                       size_t         ct_len,
                       ...
                       const uint8_t *tag)
{
    ...
    HiAEt_init_software(&state, key, nonce);
    HiAEt_absorb_software(&state, ad, ad_len);
    HiAEt_dec_software(&state, msg, ct, ct_len);
    HiAEt_finalize_software(&state, ad_len, ct_len, computed_tag);
    ...
    const uint8_t *key, const uint8_t *nonce, const uint8_t *data, size_t data_len, uint8_t *tag)
{
    HiAEt_state_t state;
    HiAEt_init_software(&state, key, nonce);
    HiAEt_absorb_software(&state, data, data_len);
    HiAEt_finalize_software(&state, data_len, 0, tag);

    return 0;
}

const HiAEt_impl_t hiaet_software_impl = { .name     = "Software",
                                           .init     = HiAEt_init_software,
                                           .absorb   = HiAEt_absorb_software,
                                           .finalize = HiAEt_finalize_software,
                                           .enc      = HiAEt_enc_software,
                                           .dec      = HiAEt_dec_software,
                                           .enc_partial_noupdate =
                                               HiAEt_enc_partial_noupdate_software,
                                           .dec_partial_noupdate =
                                               HiAEt_dec_partial_noupdate_software,
                                           .encrypt = HiAEt_encrypt_software,
                                           .decrypt = HiAEt_decrypt_software,
                                           .mac     = HiAEt_mac_software };

// End of HiAEt software implementation