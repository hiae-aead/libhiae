...
speed_test_ad_work(size_t len)
{
    ...
    memset(key, 1, HIAEX4_KEYBYTES);
    uint8_t nonce[HIAEX4_NONCEBYTES];
    memset(nonce, 1, HIAEX4_NONCEBYTES);
    ...
    do {
        HiAEx4_mac(key, nonce, ad, len, tag);
        warmup_iterations++;
...
speed_test_encode_work(size_t len, int AEAD)
{
    ...
    do {
        HiAEx4_encrypt(key, nonce, msg, ct, len, ad, ad_len, tag);
        warmup_iterations++;
        hiae_timer_stop(&warmup_timer);
    } while (hiae_timer_elapsed_seconds(&warmup_timer) < WARMUP_TIME);
...
speed_test_decode_work(size_t len, int AEAD)
{
    ...
    do {
        HiAEx4_decrypt(key, nonce, ct, dec, len, ad, ad_len, tag);
        warmup_iterations++;
        hiae_timer_stop(&warmup_timer);
    } while (hiae_timer_elapsed_seconds(&warmup_timer) < WARMUP_TIME);
...
speed_test_streaming(void)
{
    ...
    for (int i = 0; i < num_chunks; i++) {
        ...
        do {
            ...
            HiAEx4_init(&state, key, nonce);

            for (size_t j = 0; j < chunks; j++) {
                HiAEx4_enc(&state, out + j * chunk_size, data + j * chunk_size, chunk_size);
            }

            uint8_t tag[HIAEX4_MACBYTES];
            HiAEx4_finalize(&state, 0, total_size, tag);
            warmup_iterations++;
            hiae_timer_stop(&warmup_timer);
        } while (hiae_timer_elapsed_seconds(&warmup_timer) < WARMUP_TIME);
