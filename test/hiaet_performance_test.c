...
speed_test_ad_work(size_t len)
{
    ...
    uint8_t key[HIAET_KEYBYTES];
    memset(key, 1, HIAET_KEYBYTES);
    uint8_t nonce[HIAET_NONCEBYTES];
    memset(nonce, 1, HIAET_NONCEBYTES);
    ...
    do {
        HiAEt_mac(key, nonce, ad, len, tag);
        warmup_iterations++;
...
speed_test_encode_work(size_t len, int AEAD)
{
    ...
    do {
        HiAEt_encrypt(key, nonce, msg, ct, len, ad, ad_len, tag);
        warmup_iterations++;
        hiae_timer_stop(&warmup_timer);
    } while (hiae_timer_elapsed_seconds(&warmup_timer) < WARMUP_TIME);
...
speed_test_decode_work(size_t len, int AEAD)
{
    ...
    do {
        HiAEt_decrypt(key, nonce, ct, dec, len, ad, ad_len, tag);
        warmup_iterations++;
        hiae_timer_stop(&warmup_timer);
    } while (hiae_timer_elapsed_seconds(&warmup_timer) < WARMUP_TIME);
    ...
    for (int i = 0; i < num_chunks; i++) {
        ...
        do {
            ...
            for (size_t j = 0; j < chunks; j++) {
                HiAEt_enc(&state, out + j * chunk_size, data + j * chunk_size, chunk_size);
            }

            uint8_t tag[HIAET_MACBYTES];
            HiAEt_finalize(&state, 0, total_size, tag);
            warmup_iterations++;
            hiae_timer_stop(&warmup_timer);
        } while (hiae_timer_elapsed_seconds(&warmup_timer) < WARMUP_TIME);
